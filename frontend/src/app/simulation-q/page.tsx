...
const App = () => {
    ...
    const scheduleRMS = () => {
        ...
        while (currentTime < hyperperiod) {
            // Find the task with the earliest current deadline and remaining execution time
            const availableTasks = tasks.filter((task) => task.remainingTime > 0 && currentTime < task.currentDeadline);
