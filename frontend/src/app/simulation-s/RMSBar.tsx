...
function RmsBar({tasks, repetitions}: {tasks: Task[], repetitions: number}) {
    ...
    const scheduleRMS = () => {
        ...
        while (currentTime < hyperperiod) {
            // Check if any task's current deadline has passed and reset it
            tasks.forEach((task) => {
                if (currentTime >= task.currentDeadline) {
            ...
            if (availableTasks.length === 0) {
                ...
                timeline[currentTime] = null; // No task is running
                currentTime += 1;
                continue;
            }
